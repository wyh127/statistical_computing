...
```{r}
setwd("~/Desktop/ColumbiaUniversity/1/statisticalComputing/lab/2")
diamonds <- read.csv("diamonds_small.csv", as.is = TRUE, header = TRUE)
...
resample1_ests <- lm(price ~ carat, resample1_set)$coefficients
resample1_ests
```
...
names(resampled_ests) <- c("Intercept_Est", "Slope_Est")
for (b in 1:B) { 
  resampled_ests[b, ] <- lm(price ~ carat, diamonds[resampled_values[b, ], ])$coefficients
}
head(resampled_ests)
```
...
hist(resampled_ests[, 2], xlab = "Slope_Ests", main = "hist of Slope_Ests")
```
...
Cl <- quantile(diff_estimates, 0.025)
Cu <- quantile(diff_estimates, 0.975)
int <- c(Cl, Cu)
int
```
...
```{r}
quantile(resampled_values[, 2], 0.025)
quantile(resampled_values[, 2], 0.975)
```
